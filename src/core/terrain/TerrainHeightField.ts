 ...
 * (временной бюджет построек `terrain.lod.patchBuildBudgetMs`, гейт
 * `coverageReady` держит старый уровень, пока покрытие детьми не готово) —
 * в этом окне карта провиса уже посчитана под максимальный уровень, а
 * видимая сетка ещё грубее него, и холм теоретически может на глаз
 * транзиентно пройти сквозь камеру. Не лечится здесь: SSE у самой
 * поверхности требует немедленного дробления, окно грубости — доли секунды
 * построек в рамках бюджета.
 *
...
class TerrainHeightField {
  ...
  /**
   * Высота ТОЛЬКО карты, без полосы — для мест,
