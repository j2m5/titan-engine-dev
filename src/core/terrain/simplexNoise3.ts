...
const valueScratch: NoiseGrad3 = { value: 0, dx: 0, dy: 0, dz: 0 }

/** Значение симплекс-шума без градиента — обёртка над snoiseGrad3 (одно тело, бит-в-бит). */
export function simplexNoise3(vx: number, vy: number, vz: number): number {
  return snoiseGrad3(vx, vy, vz, valueScratch).value
}
