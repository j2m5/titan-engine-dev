 ...
 * файл и убирает. Шаг выводится из длины и всегда нечётный: на ширине-степени
 * двойки чётный шаг, кратный ширине, вырождал выборку в один столбец (версия
 * формата 2 — такие файлы отбрасываются по версии). Локальная правка карты
 * вне выборки по-прежнему невидима: отпечаток ловит пересборки, а не точечные
 * правки.
 */
...
export function heightMapFingerprint(map: HeightMapData): TerrainAuxFingerprint {
  const { data } = map
  // Шаг нечётный: у карт с шириной-степенью двойки (все наши) чётный шаг,
  // кратный ширине, читал бы один столбец — 8192×4096 давал ровно столбец 0.
  // Нечётный шаг взаимно прост с 2^k и обходит столбцы по кругу.
  const stride = Math.max(1, Math.floor(data.length / CHECKSUM_SAMPLES)) | 1
