 ...
 *
 * Счётчик `used` стартует не с нуля: сначала предоплачиваются РЕЗИДЕНТНЫЕ
 * ЗАКРЕПЛЁННЫЕ пути — их нельзя вытеснить в этом такте, поэтому их байты
 * заняты независимо от решения. Без этого бюджет переставал быть потолком:
 * закреплённая карта оставалась в памяти, но не считалась, и следующий такт
 * выдавал новым картам полный незанятый объём поверх уже занятого. Оплаченный
 * путь жадный проход пропускает — иначе списался бы дважды. Закреплённый
 * СИРОТА (резидент без кандидата) оплачивается, но в `wantedPaths` не входит:
 * иначе `evictOrphanedPaths` перестал бы вытеснять его по истечении пина.
 *
...
export function decideStreaming(
  ...
  // Предоплата неизбежного: резидентный закреплённый путь освободить в этом
  // такте нельзя (`evict` ниже фильтрует по `isPinned`), значит его байты
  // заняты независимо от решения. Без предоплаты `used` считал бы только
  // допущенных в этом такте кандидатов, и бюджет выдавал бы новым картам
  // полный незанятый объём ПОВЕРХ уже занятой памяти — перерасход тем
  // больше, чем плотнее группа тел.
  //
  // Осиротевшие НЕзакреплённые пути сюда не попадают намеренно: их вытесняет
  // `evictOrphanedPaths` внутри `collectCandidates`, то есть до вызова
  // политики — к этому моменту их уже нет в `loadedPaths`.
  const prepaid: Set<string> = new Set()

  for (const path of loadedPaths) {
    if (!isPinned(path)) continue

    prepaid.add(path)
    used += sizeOf(path) ?? ASSUMED_TEXTURE_BYTES
  }

  for (const candidate of ranked) {
    // Уже оплачен предоплатой и уже в памяти: признаём желаемым и
    // зарезервированным, но НЕ списываем второй раз. В `load` он не попадёт —
    // фильтр ниже отсекает всё, что уже в `loadedPaths`.
    if (prepaid.has(candidate.path)) {
