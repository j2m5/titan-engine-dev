...
class PlanetMaterial extends AbstractShaderMaterial {
  ...
  public updateMaterial(): void {
    ...
    this.defines = {
      ...
      // Облачный слой ОТКЛЮЧЁН РЕШЕНИЕМ ВЛАДЕЛЬЦА (2026-08-19, приёмочная
      // волна 2, №2): на полюсах терраформная равнопрямоугольная UV-развёртка
      // (USE_TERRAIN_UV) закручивает облачную текстуру в полосы, а прежний
      // подход «замешивать облака в диффуз» признан негодным — переосмысление
      // облачного слоя вынесено в будущую арку. USE_CLOUD больше НЕ ставится
      // ни при каком cloudMap — данные (renderingObjects/resources) и сам
      // GLSL-блок USE_CLOUD в PlanetShaderTemplate НЕ тронуты, сохранены для
      // этой будущей арки (минимальная обратимость: вернуть — одна строка).
      // cloudMap по-прежнему грузится в юниформ (см. выше) — безвредно без
      // гейта, компилятор шейдера просто не увидит блок.
    }
