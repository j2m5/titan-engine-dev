 ...
 *
 * Кодировка знаковая: байт 128 = 0, крайние 1/255 = ∓2 (tan угла) — расшифровка
 * зеркалит SLOPE_RANGE энкодера. TBN тот же, что у perturbNormalFromHeight:
 * T — восток из vEast, B = cross(N, T) — север; R-канал — уклон на восток,
 * G — на север. bumpScale — художественный множитель, 1 = физически честно.
 */
export const slopeNormalFunctions = `
  vec3 perturbNormalFromSlope(vec3 surfNormal, vec3 east, vec2 uv) {
    ...
    vec2 slope = (texture2D(bumpMap, uv).xy * 255.0 - 128.0) * (2.0 / 127.0);
