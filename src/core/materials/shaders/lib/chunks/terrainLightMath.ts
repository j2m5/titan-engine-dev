/**
 * CPU-зеркало композиции света суши (PlanetShaderTemplate, ветка USE_TERRAIN_UV):
 *   skyTerm    = mix(серый пол · sunTint, skyAmbientTint, uSkyAmbientStrength)
 *   ambient    = uTerrainAmbient · skyTerm · occlusion
 *   directGain = mix(1, occlusion, uTerrainOcclusionDirect) · cloudShadow
 *   lit        = mix(ambient, directGain · sunTint, max(N·L, 0))
 *   dayColor   = albedo · mix(sunTint, lit, uTerrainLambert)
 * sunTint — пропускание к солнцу T(μs)/T(1); небо из irradiance-LUT его уже несёт.
 * Держать синхронно с GLSL.
 */
export type Vec3 = [number, number, number]

export interface TerrainLitInput {
  ndotl: number
  ambient: number
  skyTerm: Vec3
  occlusion: number
  kDirect: number
  cloudShadow: number
  lambert: number
  /** Тинт солнца сквозь атмосферу; без атмосферы — (1, 1, 1). */
  sunTint?: Vec3
}

// Ровно форма GLSL-mix (x·(1−a) + y·a), а не a + (b−a)·t: на концах (t = 0/1)
// даёт вход бит-в-бит, иначе зеркало расходится с шейдером в последнем разряде
const mix = (a: number, b: number, t: number): number => a * (1 - t) + b * t

export function terrainLit(i: TerrainLitInput): Vec3 {
  const l = Math.max(i.ndotl, 0)
  const tint = i.sunTint ?? [1, 1, 1]
  const directGain = mix(1, i.occlusion, i.kDirect) * i.cloudShadow
  const lit = i.skyTerm.map((s: number, c: number): number => mix(i.ambient * s * i.occlusion, directGain * tint[c], l)) as Vec3
  return lit.map((v: number, c: number): number => mix(tint[c], v, i.lambert)) as Vec3
}

/** Небо над сушей: серый пол — отражённое солнце (с тинтом), LUT-небо — без. */
export function terrainSkyTerm(greyFloor: number, sunTint: Vec3, skyAmbient: Vec3, skyStrength: number): Vec3 {
  return sunTint.map((t: number, c: number): number => mix(greyFloor * t, skyAmbient[c], skyStrength)) as Vec3
}

export interface ComposeInput {
  night: Vec3
  cloudColor: Vec3
  cloudAlpha: number
  dayColor: Vec3
  dayFactor: number
  lambert: number
}

/** Композиция кадра суши: облака под dayFactor, суша под landGate, ночь под (1 − dayFactor). */
export function composeTerrain(i: ComposeInput): Vec3 {
  const landGate = mix(i.dayFactor, 1, i.lambert)
  return i.night.map((n: number, c: number): number =>
    n * (1 - i.dayFactor) + i.cloudColor[c] * i.dayFactor + i.dayColor[c] * (1 - i.cloudAlpha) * landGate
  ) as Vec3
}

export const CLOUD_SHADOW_MIN_COS = 0.15

/**
 * Сдвиг uv тени облака: облако, затеняющее точку, стоит по направлению к солнцу на h·tan θ;
 * sunTangent — касательная проекция направления на солнце в базисе (east, north) (длины в единицах),
 * u делится на 2πR·cos φ, v — на πR. cos θ клампится снизу — без разлёта у терминатора.
 */
export function cloudShadowUvOffset(
  sunTangentEN: [number, number],
  muS: number,
  heightUnits: number,
  radiusUnits: number,
  cosLat: number
): { du: number; dv: number } {
  const cosZ = Math.max(muS, CLOUD_SHADOW_MIN_COS)
  const scale = heightUnits / cosZ
  return {
    du: (sunTangentEN[0] * scale) / (2 * Math.PI * radiusUnits * Math.max(cosLat, 1e-3)),
    dv: (sunTangentEN[1] * scale) / (Math.PI * radiusUnits)
  }
}

/** Легаси-композиция: mix(night, cloud + day·(1−α), dayFactor). */
export function composeLegacy(i: ComposeInput): Vec3 {
  return i.night.map((n: number, c: number): number =>
    mix(n, i.cloudColor[c] + i.dayColor[c] * (1 - i.cloudAlpha), i.dayFactor)
  ) as Vec3
}
