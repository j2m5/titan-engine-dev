 ...
 * selectTerrainNodes): пик живых патчей 72 (Земля, H=1080), 84 (Земля,
 * H=2160), 60 (Луна, H=1080) — везде патчи упираются в
 * TERRAIN_QUADTREE_MAX_LEVEL раньше, чем успевают набрать сотни (в отличие
 * от рельефа, потолок глубины у воды достигается почти сразу у поверхности
 * и дальше не растёт с приближением). 256 — операционный запас (>3× пика)
 * при этом на порядок меньше террейнового потолка.
 */
 ...
 * хорды вершинного шага уровня вместо честного p99 размаха высот — числитель
 * SSE ненулевой и убывает с глубиной уровня, отбор самотерминируется в
 * фактической SSE-метрике selectTerrainNodes БЕЗ отдельной ручки (крупное
 * тело делит глубже, мелкое — мельче, из космоса везде 24 листа — SSE
 * MIN_LEVEL уже ниже порога). До фикса ε константного поля был тождественно
 * 0 (найдено ревью Task 3, фикс-раунд 1) — SSE не пробивал порог НИКОГДА,
 * дерево оставалось на MIN_LEVEL везде: посадка на воду видела гранёную
 * поверхность в сотни метров провиса вместо аналитического пола коллизии.
 *
