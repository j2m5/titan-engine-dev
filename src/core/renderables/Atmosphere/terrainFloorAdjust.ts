 ...
 * а не вкус (хотфикс 2026-08-17, подтверждено спайком 2026-08-22). Слой между
 * новым дном и датумом несёт плотность датума (constantTerm: 1, кламп [0,1]
 * даёт ρ≡1) — не ноль: атмосфера режет лучи по реальной глубине сцены
 * (AtmosphereEffect), и подповерхностный слой попадает только в лучи, реально
 * идущие под датум (котловины), и в члены LUT второго порядка. Замер спайка:
 * диск с орбиты ±1 %, в котловинах появляется воздушная перспектива вместо
 * жёсткого обрыва дымки на горизонте. Экспонента под датумом тождественна
 * константе только благодаря клампу — явная константа не зависит от клампа.
 */
...
function adjustSpecies(profile: Profile, dKm: number): Profile {
  ...
  if (isExpProfile(profile)) {
    ...
    //
    // Нижний слой — константная прокладка на плотности датума (ρ≡1) толщиной
    // d. Атмосфера режет лучи по реальной глубине сцены, поэтому этот слой
    // видят только лучи, реально идущие под датум (котловины), и члены LUT
    // второго порядка — не каждый луч к диску, как было при аналитическом дне
    // хотфикса 2026-08-17. Замер спайка 2026-08-22: диск с орбиты сдвигается
    // на доли процента, в котловинах появляется физически осмысленная дымка
    // вместо вакуума.
    const factor = Math.exp(dKm * -profile[1].expScale)

    return [
      { width: dKm, expTerm: 0, expScale: 0, linearTerm: 0, constantTerm: 1 },
