 ...
 * идущие под датум (котловины), и в члены LUT второго порядка: диск с
 * орбиты не меняется, в котловинах есть воздушная перспектива вместо
...
function adjustSpecies(profile: Profile, dKm: number): Profile {
  ...
  if (isExpProfile(profile)) {
    ...
    // второго порядка — не каждый луч к диску. Нулевая прокладка оставляла бы
    // котловины без дымки.
