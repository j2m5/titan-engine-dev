...
const enum LODLevel {
   ...
   * архетип, свой detail). Пул уже держит для неё отдельные стримы
   * (см. nearStreamBase), но МЕНЕДЖЕР её пока не выбирает — адресация
   * начинается в Task 2 плана «2c — ближний тир».
   */
  GeometryNear = 2
}
 ...
 *
 * Владеет 2K+1 рендер-объектами:
 * - K стримов Geometry (0..K-1): по одному InstancedMesh на архетип, обычный detail.
 * - K стримов Near (K..2K-1): те же архетипы, свой (обычно более высокий)
 *   detail — отдельная библиотека геометрий и своя ёмкость (nearConfig).
 *   Geometry и Near ДЕЛЯТ один InstancedAsteroidMaterial с Geometry — профильные
 *   юниформы едины для обоих тиров реальной геометрии (иначе драуколлов было
 *   бы 2K+1 материалов вместо 2).
 * - 1 стрим Billboard (индекс 2K): InstancedMesh с PlaneGeometry (camera-facing).
 *
 ...
 *
 * Итог: 2K+1 draw calls на всю систему экземпляров.
 */
class InstancePool {
   ...
   * @param nearGeometries K геометрий Near-архетипов (та же библиотека архетипов,
   *   другой detail). Длина ОБЯЗАНА совпадать с l0Geometries — иначе адресация
   *   архетипа k в двух тирах разъехалась бы (сектор меняет ТОЛЬКО detail при
   *   переходе L0↔Near, архетип/семя остаются теми же).
