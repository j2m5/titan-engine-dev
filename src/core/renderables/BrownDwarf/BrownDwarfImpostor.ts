 ...
 * zonalShear, fineDetail), что и диск, а палитра и ручки копируются из
 * материала тела — на дистанции переключения уровни совпадают по построению,
 * ручек подстройки нет намеренно (тот же контракт, что у FakeStar).
 *
