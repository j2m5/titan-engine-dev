 ...
 * отсчёты лежат на КРАЯХ домена: первый узел стоит ровно на b = B_MIN,
 * последний — ровно на краю зоны, со значением 0 (не в центрах текселей).
 * Причина: ClampToEdge + LinearFilter делают выборку плоской на внешних
 * полутекселях, и при сетке по центрам эти плоские участки приходились
 * ровно на оба шва — силуэт зоны и стык с живым интегратором. Значение —
 * угол (рад) между конечным и начальным направлением луча, накопленный
 * ВНУТРИ зоны: на краю зоны хорда нулевая, α = 0, и непрерывность с
 * нелензированным фоном вне меша автоматическая, без окон и кроссфейдов.
 */
...
export function bakeDeflectionAngles(simulationRs: number, dphi: number): Float32Array {
  ...
  // Последний узел — точный нулевой предел хорды (край зоны), а не интеграл:
  // живой интегратор в шейдере на b = simulationRs не запускается (LUT-ветка
  // кончается раньше меша), так что параметрическое совпадение здесь не
  // нужно, а однопроходная secant-погрешность (~0.0014 рад) — это ровно тот
  // шов на силуэте, ради устранения которого затевалась краевая сетка
  angles[DEFLECTION_LUT_SIZE - 1] = 0
