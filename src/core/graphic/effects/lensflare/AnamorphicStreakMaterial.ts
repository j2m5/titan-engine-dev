 ...
 *
 * Размытие здесь только по X (см. `vec2(texelSize.x * spread * ..., 0.0)`
 * ниже) — вертикальную толщину черты проход целиком наследует от входного
 * буфера, своего вертикального blur у него нет. Поэтому `inputBuffer`
 * обязан быть резким (пороговый буфер до Kawase-предразмытия в
 * `LensFlareEffect.update()`), а не уже размытым: предразмытый источник
 * раздувает черту по вертикали ещё до анаморфной растяжки.
 */
