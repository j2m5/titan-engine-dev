...
const fragmentShader: string = `
  ...
  // Цвет призрака — из одномерного градиента по радиусу от центра кадра, чтобы
  // палитра всех девяти отражений принадлежала объективу.
  // Нормировка обязана совпадать с той, что в sampleGhost: иначе радиусы здесь
  // и в спаде расходятся вдвое и правая половина градиента не читается никогда
  vec3 ghostTint(const vec2 suv) {
    float d = clamp(length(vec2(0.5) - suv) / (0.5 * SQRT_2), 0.0, 1.0);
    return texture(lensColor, vec2(d, 0.5)).rgb;
  }

  // Порог вычитается с отсечкой в ноль, а не гасит выборку гладким множителем:
  // вычитание убирает постоянную составляющую, множитель сохранял бы форму.
  // Идёт ДО тонировки — порог по затонированному цвету резал бы палитру
  // градиента, а не значения буфера поканально.
  //
  // Широкую яркую площадь этот порог не убирает: её отсекает проход локального
  // контраста, через который буфер проходит раньше. Здесь порог работает уже по
  // разностям с окрестностью и подчищает остатки
  vec3 sampleGhost(const vec2 direction, const float weight, const float offset) {
    ...
    // Затухание к периметру кадра: чем больше показатель, тем плотнее призраки
    // жмутся к центру
  ...
  // Лучи объектива: маска повёрнута по ориентации камеры и модулирует уже
  // посчитанные артефакты, к источнику света не привязана.
  // Коррекция по vAspectRatio переводит координаты в квадратное пространство
  // перед поворотом — без неё на неквадратном экране получится не вращение, а
  // сдвиговое искажение
  ...
  void main() {
    ...
    features *= 1.0 + starburstAmount * sampleStarburst();

    // Штрих идёт ЧЕРЕЗ яркий пиксель, а не зеркалится через центр, поэтому
    // выборка по тому же uv. Оттенок применён в проходе штриха, второй раз
    // умножать нельзя.
    //
    // Порядок значим: штрих прибавляется ПОСЛЕ множителя маски и ею не
    // модулируется. Маска повёрнута по крену камеры, так что внутри неё по
    // полосе ездили бы яркие и тусклые секторы, а штрих — свойство
    // горизонтальной оптики и вращаться не должен
    features.rgb += texture(streakBuffer, vUv).rgb * streakAmount;

    gl_FragColor = features;
  }
`
