 ...
 *
 * Терраформный контакт двухфазный по РАССТОЯНИЮ, не по этапу обработки
 * (фикс карты провиса, `TerrainHeightField`, три раунда): широкая фаза
 * (`collectColliders`) — всегда консервативная сфера (R+maxH+maxClearance).
 * Внутри неё `marchTerrain` дальше выбирает поверхность ПО МЕСТУ вдоль
 * свипа: снаружи консервативной оболочки сетки провиса — марч против неё
 * (гарантия отсутствия туннеля на быстром пролёте, сетка размазывает
 * пиковый провис по ячейке+дилатации и потому дороже честной, но дёшева и
 * безопасна для больших скачков); как только отрезок ВОШЁЛ в эту оболочку
 * (стартовал внутри или пересёк её по пути) — марч продолжает против
 * честной поточечной поверхности (`TerrainHeightField.sagMeters`,
 * `marchPointwise`), потому что оболочка — теперь штатная рабочая высота
 * камеры (медиана на Луне ~236 м после сетки против ~40 м честно
 * поточечно), а не редкий крайний случай: пропускать её маршем целиком
 * открывало туннель на низких быстрых пролётах (раунд 3). `pushOutTerrain`
 * — та же поточечная цель (см. `pointwiseFloorRadiusUnits`), как и раньше.
 */
class CameraCollision {
   ...
   *
   * Старт УЖЕ внутри консервативной оболочки (`distance(from) <= 0`) — раунд 2
   * фикса карты провиса сделал эту оболочку штатной рабочей высотой камеры
   * (честный пол по sagMeters ~40 м медианно при оболочке сетки ~236 м), не
   * редким крайним случаем: старая логика «пропустить марч целиком, отдать
   * пуш-аут» здесь ОТКРЫВАЛА ТУННЕЛЬ на быстром тангенциальном пролёте низко
   * над рельефом (пуш-аут ловит только СТАРТОВУЮ точку, не путь между from и
   * to). Вместо пропуска — марч сразу в поточечном режиме на полный внешний
   * бюджет, см. `marchPointwise`.
   */
   ...
   * полный марч, когда старт уже внутри консервативной оболочки (см.
   * докблок `marchTerrain` — это раунд 3, закрывающий дыру раунда 2).
   *
   * Липшицева константа шага: SLOPE_RANGE (уклон DEM, как и у внешнего
   * марча) ПЛЮС `field.sagSlopeBond` — консервативный бонд уклона самого
   * sag-поля (maxSagMeters/texelMeters, см. TerrainHeightField), потому что
   * поточечная поверхность = h + sag, и её суммарный градиент бондится
   * суммой бондов обеих компонент. Шаг d/(1+L) соответственно короче, чем у
   * внешнего марча — при том же бюджете (SWEEP_MARCH_BUDGET=64 в режиме (б))
   * это означает МЕНЬШИЙ гарантированный радиус сходимости за проход, но
   * зазор здесь тоже меньше на тот же множитель (искомая поверхность —
   * честный пол, а не консервативная оболочка): для типичных скоростей
   * камеры у поверхности (низкий пилотируемый пролёт, не варп) 64 шагов с
   * запасом хватает — подтверждено замером на реальной карте (см. хендофф).
   * `hardStopOnExhaustion` — true для режима (б) (потенциально длинный/быстрый
   * отрезок, риск как у внешнего марча), false для режима (а) (короткий
   * гарантированный довесок, сходимость должна была хватить).
   */
  private marchPointwise(
    ...
    const totalSlopeBond = SLOPE_RANGE + field.sagSlopeBond
