 ...
 *
 * Методика калибровки — ДВУХкритериальная: физическая привязка одна,
 * проверка две.
 *
 ...
 * телами движка (ориентиры: легаси-bump ≈ 0.034, реальный DEM Луны ≈ 0.14;
 * цель Каллисто — 0.05–0.09) — иначе карта визуально плоская даже при
 * «правильной» глубине кратеров. `--band-high-km` — рычаг ПЛОТНОСТИ
 ...
 * дотягивает до 0.05 и на треть (bump-полоса этого тела слишком гладкая).
 * Приоритет отдан RMS(tan): amplitude 8400 м — осознанное художественное
 * преувеличение (та же природа, что легаси-bumpScale > 1), глубина p99-фичи
 * и диапазон высот карты выходят за литературные. Подгонка знака и силуэта —
 ...
 * морфология). `bandHighKm` — пол по полосе (2 текселя экватора карты-пилота,
 * 4096×2048, ниже — JPEG-зерно), `bumpAmplitudeMeters` — до целевого
 * RMS(tan) 0.05–0.09 по двухкритериальной методике из докблока CLI.
 */
