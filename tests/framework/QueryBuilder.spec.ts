...
describe('QueryBuilder — глобальные скоупы', () => {
  it('count() и get() уважают глобальный скоуп', () => {
    ;(User as any).addGlobalScope?.('not_deleted', new NotDeletedScope())

    // Если addGlobalScope недоступен под этим именем — тест помечается зависимым
    // от реализации регистрации скоупов; см. примечание в конце файла.
    if ((User as any).getGlobalScopes?.().size) {
      expect(
        User.query()
          .get()
          .pluck('id' as any)
