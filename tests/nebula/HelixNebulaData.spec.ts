...
describe('туманность Helix — форма', () => {
  ...
  it('это оболочка, а не плоский диск', () => {
    // Диск с вертикальной отсечкой вдвое круче радиальной читался блином.
    // Сначала оболочка собиралась эллипсоидом с двумя кавернами — это был обход
    // отсутствовавшего примитива; теперь она собрана примитивом shell напрямую.
    // Оба варианта проверку проходят, потому что важна не форма-имя, а профиль
    // (см. тест ниже). Возврат к 'disk' — возврат к унынию
    expect(['shell', 'ellipsoid']).toContain(params.shape)
