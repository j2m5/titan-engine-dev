...
describe('decideStreaming', () => {
  ...
  it('закреплённый путь, который И кандидат, списывается один раз', () => {
    // Guard, а не RED: двойного списания не существует, пока нет предоплаты —
    // на старом коде тест зелёный по построению. Ловит он сломанный вариант
    // правки: «предоплата есть, а пропуск оплаченного в жадном проходе забыли».
    // Закреплённый 'p' намеренно НЕ пол (slope не топ-тела): пол обошёл бы
    // проверку бюджета и замаскировал повторное списание.
    //
    // Бюджет 300 по 100 за карту. Предоплата 'p' = 100, пол 'top.diff' = 100,
    // остаётся ровно 100 на 'q'. При повторном списании 'p' места на 'q' нет.
    const d = decideStreaming(
      [mc(1, 'top.diff', 0, 5), mc(2, 'p', 1, 2), mc(3, 'q', 2, 1)],
      new Set(['p']),
      (path: string): boolean => path === 'p',
      noneExcluded,
      size(100),
      300
    )

    expect(d.load.map((c: MapCandidate): string => c.path)).toEqual(['top.diff', 'q'])
    // уже в памяти — грузить нечего, но путь остаётся желаемым и оплаченным
    expect(d.wantedPaths.has('p')).toBe(true)
  })
