...
describe('PlanetShaderTemplate: терминатор суши без двойного гашения', () => {
  ...
  it('обе ветки под одним #ifdef USE_TERRAIN_UV / #else; тинт суши — внутри dayColor, легаси — на всём day', () => {
    ...
    // У суши тинт (sunTintMix) уже в dayColor и облаках — второго множителя на day нет
    ...
    expect((terrain.match(/sunTint\(muS\)/g) ?? []).length).toBe(0)
    expect(terrain).not.toMatch(/day \*=/)
