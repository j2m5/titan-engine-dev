...
describe('PlanetShaderTemplate: декод cavity-канала (строковые ассерты)', () => {
  ...
  it('выборка cavity стоит ПОСЛЕ perturbNormalFromSlope и ДО applyTerrainDetail', () => {
    const slopeIdx = frag.indexOf('perturbNormalFromSlope(nLocal, eastLocal, uv, vMidTilt, terrainMapSlopeVec)')
...
// Счётный инвариант данных (Task 3, дополнен Task 6 аркой воды и Task 4 —
// система W26): охват выводится ПРОГРАММНО из БД — акторы с height-ресурсом
// (путь заканчивается на _height.raw), минус фотомозаичные {5, 6, 8, 19, 7}
// (Меркурий, Венера, Марс, Луна, Земля — реальные снимки/DEM, cavity им не
// полагается). Ожидание — ровно 49 тел (45 прежних + 4 процедурных тела W26:
// Emberon, Halcyra I/II, Nivalis).
describe('Счётный инвариант: cavityStrength у 49 терраформных тел (Task 3 + Task 6 + Task 4)', () => {
