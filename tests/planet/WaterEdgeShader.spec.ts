...
describe('PlanetShaderTemplate: мокрая кромка берега (USE_WATER_EDGE)', () => {
  it('атрибут height и varying объявлены под объединённым гейтом полосы/кромки', () => {
    ...
    expect(frag).toContain('#if defined(USE_TERRAIN_MACRO_DETAIL) || defined(USE_WATER_EDGE)')
  })
  ...
  it('глинт: общая функция для USE_SPECULAR и кромки, одно тело', () => {
    ...
    const wetGlint = frag.indexOf('finalColor += wetEdge * blinnPhongGlint(normal, lightDirection, viewDir) * WET_GLOSS')
...
describe('PlanetMaterial: дефайн USE_WATER_EDGE и юниформы кромки', () => {
  ...
  it('Земля (уровень 0, slope и macroStrength): дефайн стоит вместе с USE_TERRAIN_MACRO_DETAIL', () => {
    const earth = Actor.find(7)!
    seedFor(earth)
    const material = new PlanetMaterial(earth)
    // без карты высот в реестре рельефные дефайны молчат — тогда и кромка молчит
    if (material.defines.USE_TERRAIN_MACRO_DETAIL === '1') {
