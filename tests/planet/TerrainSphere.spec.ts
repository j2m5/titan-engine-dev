...
import { makeFrameClock } from '../terrain/frameClock'
...
describe('TerrainSphere: динамическое квадродерево патчей', { timeout: 30000 }, () => {
  ...
  // Качающаяся камера: желаемая глубина гуляет на порядки, поэтому в живых
  // постоянно остаются узлы, построенные под прежнюю глубину и ещё не
  // показанные. Инвариант держат обе ветки показа — спуск в кадр освобождения
  // замещаемого узла и страховка «скрыт ∧ нет живого предка → показать»;
  // двойное покрытие (видимый предок над видимыми потомками) рулингом
  // допустимо, дыра — нет.
