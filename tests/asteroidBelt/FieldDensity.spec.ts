...
describe('Поле пояса изнутри: что реально перед камерой', () => {
  it('камера в средней плоскости на 50 а.е. — плотное поле, а не пустота', () => {
    ...
    // Крупные тела различимы издалека (замерено 248 крупнее 10 px; прежние
    // 1 365 считали хвосты единичных матриц — тела масштаба 1 в началах секторов)
    expect(field.largerThanPx(10)).toBeGreaterThan(150)
