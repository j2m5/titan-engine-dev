...
describe('рассеяние кольца', () => {
  ...
  it('лепесток вперёд не сужен до игольчатого — соседние cosTheta не дают скачка', () => {
    // Функция гладкая по построению (зеркало не знает о сторонах кольца);
    // порог ловит вырождение лепестка в иглу при forwardScattering, близком к 1
    const step = 0.001
    let maxJump = 0
